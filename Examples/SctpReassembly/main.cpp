...
static struct option SctpAssemblyOptions[] = {{"interface", required_argument, 0, 'i'},
...
typedef std::map<std::string, SctpReassemblyData> SctpReassemblyMgr;
typedef std::map<std::string, SctpReassemblyData>::iterator SctpReassemblyMgrIter;
...
static void OnSctpMessageReadyCallback(pcpp::SctpPacketData *sctpData, void *userCookie)
{
/* 	1. manager 存 SctpReassemblyData   									yes
	2. manager 的指定 SctpReassemblyData 里边fileStream 是否为NULL
		2.1 将当前（指传入的参数）的名称加入opened列表
		2.2 如果打开的文件已达上限，关闭目前的
		2.3 设置文件名
		2.4 打开文件， 模式由之前2.2设置的reopenFileStreams决定
	3. 更改SctpReassemblyData里的统计值
	4. 将数据写入打开的文件里
 */
	...
	SctpReassemblyMgr *mgr = (SctpReassemblyMgr *)userCookie;

	// check if this tuple already appears in the manager. If not add it
	SctpReassemblyMgrIter iter = mgr->find(sctpData->getTupleName());
	if (iter == mgr->end())
	{
		mgr->insert(std::make_pair(sctpData->getTupleName(), SctpReassemblyData()));
		iter = mgr->find(sctpData->getTupleName());
	}
	...
	if (iter->second.fileStream == NULL)
	{
		...
		int result =
			GlobalConfig::getInstance().getRecentFilesWithActivity()->put(sctpData->getTupleName(), &nameToCloseFile);
		...
		if (result == 1)
		{
			SctpReassemblyMgrIter iter2 = mgr->find(nameToCloseFile);
			if (iter2 != mgr->end())
			{
		...
		std::string name = sctpData->getTupleName() + ".txt";
	...
	iter->second.bytes += (int)sctpData->getDataLength();

	// 4. 

	// write the new data to the file
	iter->second.fileStream->write((char*)sctpData->getData(), sctpData->getDataLength());
 ...
 */
static void onPacketArrives(pcpp::RawPacket *packet, pcpp::PcapLiveDevice *dev, void *sctpReassemblyCookie)
{
	// get a pointer to the TCP reassembly instance and feed the packet arrived to it
	pcpp::SCTPReassembly *sctpReassembly = (pcpp::SCTPReassembly *)sctpReassemblyCookie;
	sctpReassembly->reassemblePacket(packet);
}

//++++++ok
/**
 * The method responsible for SCTP reassembly on pcap/pcapng files
 */
void doSctpReassemblyOnPcapFile(std::string fileName, pcpp::SCTPReassembly &sctpReassembly, std::string bpfFilter = "")
{
	...
	{
		sctpReassembly.reassemblePacket(&rawPacket);
	}
...
//++++++ok
/**
 * The method responsible for SCTP reassembly on live traffic
 */
void doSctpReassemblyOnLiveTraffic(pcpp::PcapLiveDevice *dev, pcpp::SCTPReassembly &sctpReassembly,
								  std::string bpfFilter = "")
{
	...
	dev->startCapture(onPacketArrives, &sctpReassembly);
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "i:r:o:e:f:mcvhl", SctpAssemblyOptions, &optionIndex)) != -1)
	{
		switch (opt)
		{
		case 0:
	...
	SctpReassemblyMgr mgr;

	//  create the TCP reassembly instance
	pcpp::SCTPReassembly sctpReassembly(OnSctpMessageReadyCallback, &mgr);

	// analyze in pcap file mode
	if (!inputPcapFileName.empty())
	{
		doSctpReassemblyOnPcapFile(inputPcapFileName, sctpReassembly, bpfFilter);
	}
	else // analyze in live traffic mode
	{
		...
		doSctpReassemblyOnLiveTraffic(dev, sctpReassembly, bpfFilter);
	}
}